...
import { createProjectList } from "@/app/actions/projects";

interface ProjectListFormProps {
  projectId: string;
}

export default function ProjectListForm({ projectId }: ProjectListFormProps) {
  // Bind projectId as the first parameter
  const createListWithProjectId = createProjectList.bind(null, projectId);
  const [state, formAction, isPending] = useActionState(createListWithProjectId, undefined);
  ...
  return (
    <form ref={formRef} action={formAction} style={{ display: "flex", flexDirection: "column", gap: "0.75rem", maxWidth: "480px" }}>
      ...
      <div style={{ display: "flex", gap: "0.75rem" }}>
        <input
          name="title"
          placeholder="New list title (e.g. Phase 1 Tasks)..."
          required
          disabled={isPending}
