...
export default function ListForm() {
  const [state, formAction, isPending] = useActionState(createList, undefined);
  const formRef = useRef<HTMLFormElement>(null);

  // Clear the input when list is successfully created
  useEffect(() => {
  ...
  return (
    <form ref={formRef} action={formAction} style={{ display: "flex", flexDirection: "column", gap: "0.75rem", maxWidth: "480px" }}>
      ...
      <div style={{ display: "flex", gap: "0.75rem" }}>
        <input
          name="title"
          placeholder="Enter list title (e.g. Read List, Project Specs)..."
          required
          disabled={isPending}
